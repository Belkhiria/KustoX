...
exports.registerComprehensiveTests = exports.ComprehensiveTestRunner = exports.COMPREHENSIVE_TEST_SUITE = void 0;
const vscode = __importStar(require("vscode"));
const resultCapture_1 = require("./resultCapture");
exports.COMPREHENSIVE_TEST_SUITE = [
    {
...
| summarize TotalEvents = count(), TotalDamage = sum(DamageProperty) by EventType, State`,
        validate: (result) => {
            ...
            if (!resultCapture_1.ResultValidator.validateColumns(result, expectedCols)) {
                return {
                    passed: false,
...
class ComprehensiveTestRunner {
    ...
    async runAllTests() {
        ...
        this.testResults = [];
        const resultCapture = resultCapture_1.KustoXResultCapture.getInstance();
        for (let i = 0; i < exports.COMPREHENSIVE_TEST_SUITE.length; i++) {
            const testCase = exports.COMPREHENSIVE_TEST_SUITE[i];
            const startTime = Date.now();
            console.log(`\n🧪 Test ${i + 1}/${exports.COMPREHENSIVE_TEST_SUITE.length}: ${testCase.name}`);
    ...
    async runSingleTest(testCase, resultCapture) {
        try {
    ...
    generateMockResult(testCase) {
        // Generate appropriate mock results based on the test case
        if (testCase.name === "basic_take_query") {
            return resultCapture_1.MockResultGenerator.generateStormEventsResult(10);
        }
        else if (testCase.name === "your_problem_query") {
            return resultCapture_1.MockResultGenerator.generateAggregationResult();
        }
        else if (testCase.name === "simple_print") {
            return resultCapture_1.MockResultGenerator.generatePrintResult("Hello KustoX");
        }
        else if (testCase.name === "empty_result_query") {
            return resultCapture_1.MockResultGenerator.generateEmptyResult(['EventType', 'State', 'DamageProperty']);
        }
        else if (testCase.name === "error_handling_test") {
            return resultCapture_1.MockResultGenerator.generateErrorResult("Syntax error: invalid_operator is not recognized");
        }
        else if (testCase.name === "large_result_handling") {
            return resultCapture_1.MockResultGenerator.generateStormEventsResult(100);
        }
        else {
...
function registerComprehensiveTests(context) {
...
exports.registerComprehensiveTests = registerComprehensiveTests;
//# sourceMappingURL=comprehensiveTests.js.map