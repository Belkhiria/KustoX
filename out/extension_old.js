...
| take 10`
    ...
    function analyzeKustoQuery(queryLower, lines) {
        const analysis = {
            tableName: null,
            columns: [],
            operations: [],
    ...
    function generateTableSpecificResults(analysis) {
    ...
    function generateStormEventsData(analysis) {
    ...
    function generateLogData(analysis) {
    ...
    function generatePerformanceData(analysis) {
    ...
    function generateSecurityData(analysis) {
    ...
    function generateGenericTableData(analysis) {
        ...
        for (let i = 0; i < Math.min(6, analysis.limit || 6); i++) {
            const row = columns.map((_, index) => `Value_${i + 1}_${index + 1}`);
            rows.push(row);
        }
        return { columns, rows };
    }
    function generateGenericResults(analysis) {
        return {
            columns: ['Result'],
    ...
    function generateExecutionTime(rowCount) {
    ...
    function getResultsWebviewContent(query, results) {
        const tableRows = results.rows.map((row) => `<tr>${row.map(cell => `<td>${cell}</td>`).join('')}</tr>`).join('');
